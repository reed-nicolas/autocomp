        ...
        for c_in_tile_idx in nl.affine_range(n_tiles_c_in):
            for i in nl.affine_range(filter_height):
                for j in nl.affine_range(filter_width):
                    ...
                    # YOUR CODE HERE
                    w_filter = nl.load(W[c_out_tile_idx * c_out_tile: (c_out_tile_idx + 1) * c_out_tile, c_in_tile_idx * c_in_tile: (c_in_tile_idx + 1) * c_in_tile, :, :])
                    w_tile = w_filter[:, :, i, j]
    ...
    for img in nl.affine_range(batch_size):
        ...
        for c_out_tile_idx in nl.affine_range(n_tiles_c_out):
            ...
            for out_row in nl.affine_range(out_height):
                ...
                # YOUR CODE HERE
                b = nl.load(bias[c_out_tile_idx * c_out_tile + nl.arange(c_out_tile)[:, None]])
                output = nl.copy(row_out, dtype=X_out.dtype) + b

                # Store the output  
                # YOUR CODE HERE
                nl.store(X_out[img, c_out_tile_idx * c_out_tile: (c_out_tile_idx + 1) * c_out_tile, out_row, :], value=output)
